...
```{r figure 5, echo=FALSE, out.width='100%', fig.height=6, fig.width = 8}
...
```{r figure6(ii), echo=FALSE, out.width='80%', fig.height=6}
...
plot <- ggplot(data, aes(x = variable_mapped, y = value, fill = variable_mapped)) +
    ...
    geom_text(aes(label = scales::percent(value), y = value, group = variable_mapped), 
              position = position_stack(vjust = 0.5), size = 3) +
    labs(title = "Non-eligible persons covered in trainings (n=47)", x = NULL, y = NULL) +
    coord_flip() +
    theme_minimal() +
    theme(axis.text.x = element_blank()) + # Remove x-axis text
  scale_x_discrete(labels = function(x) str_wrap(x, width = 20))
...
```{r figure8, echo=FALSE, out.width='100%', fig.height=6, fig.width = 8}
library(ggplot2)
library(gridExtra)
library(stringr)
...
```{r figure9, echo=FALSE, out.width='100%', fig.height=6, fig.width = 8}

library(ggplot2)
library(gridExtra)
library(stringr)

# sql query to get data (messages covered under commmunity sensitization)
sql_query <- "SELECT * FROM tt_obs
WHERE year = 2022
AND round = 'Overall'
AND state = 'Rivers'
