...
plot <- ggplot(data, aes(x = attributes, y = value, fill = attributes)) +
     ...
     geom_text(aes(label = scales::percent(value), y = value, group = attributes), 
               position = position_stack(vjust = 0.5), size = 3) +
     labs(title = "FLHF knowledge of side effects on deworming day (n=14)", x = NULL, y = NULL) +
     coord_flip() +
     theme_minimal() +
     theme(axis.text.x = element_blank()) +  
    scale_x_discrete(labels = function(x) str_wrap(x, width = 20)) 
...
The majority of the parents heard about deworming from either their child (74%) or a teacher (40%). Most parents prefer getting deworming information from the town announcer (49%), radio (45%), teachers (42%), and children (38%) - **Figure 17**.
...
Seventy-three percent (73%) of parents around school were aware of deworming, and 76% of these parents indicated that they would send at least one of their children for deworming. The reasons for not sending children for deworming can be found in - Figure 18.
